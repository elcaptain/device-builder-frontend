...
describe("ESPHomeAPI — getBoardBodies", () => {
  beforeEach(() => {
    installMockWebSocket();
  });
  afterEach(() => {
  ...
  it("sends ``boards/get_board_bodies`` with the requested ids and hydrates each body", async () => {
    const api = new ESPHomeAPI();
    const ws = await connect(api);

    const pending = api.getBoardBodies(["esp32_devkit_v1", "d1_mini"]);
    const sent = ws.sentAs<{ command: string; args: Record<string, unknown> }>(0);

    expect(sent.command).toBe("boards/get_board_bodies");
    expect(sent.args).toEqual({ ids: ["esp32_devkit_v1", "d1_mini"] });

    // Stripped wire shape — the body returned by the backend's
    // omit_default config omits default_components / pins /
    // featured_*; the hydrator must re-default each one.
    const payload = {
      esp32_devkit_v1: {
        id: "esp32_devkit_v1",
      ...
      d1_mini: {
        ...
        description: "",
        manufacturer: "Wemos",
        esphome: { platform: "esp8266", board: "d1_mini" },
      },
    };
    ws.receive({
      message_id: ws.sentAs<{ message_id: string }>(0).message_id,
    ...
    const result = await pending;
    expect(Object.keys(result)).toEqual(["esp32_devkit_v1", "d1_mini"]);
    expect(result.esp32_devkit_v1.pins).toEqual([]);
    expect(result.esp32_devkit_v1.featured_components).toEqual([]);
    expect(result.esp32_devkit_v1.default_components).toEqual([]);
    expect(result.d1_mini.pins).toEqual([]);
  });

  it("short-circuits on an empty id list without touching the socket", async () => {
    const api = new ESPHomeAPI();
    const ws = await connect(api);
    const result = await api.getBoardBodies([]);
    expect(result).toEqual({});
    expect(ws.sent).toHaveLength(0);
  });

  it("logs an error when the backend omits a requested id", async () => {
    const api = new ESPHomeAPI();
    const ws = await connect(api);
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});

    const pending = api.getBoardBodies(["known", "missing"]);
    ws.receive({
      message_id: ws.sentAs<{ message_id: string }>(0).message_id,
      result: {
        known: {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      expect.stringContaining("backend omitted bodies"),
      ["missing"]
    );
